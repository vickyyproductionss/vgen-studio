...
export default function BeatSync({ projectId, onStartRender }: BeatSyncProps) {
  ...
  // Prefetch all clip video URLs into browser cache whenever clip assignments change.
  // This eliminates the 1-second stutter when the player transitions to a new scene
  // (without this, each clip is fetched cold on-demand during playback).
  const prefetchHandles = useRef<Array<{ free: () => void }>>([]);
  useEffect(() => {
    // Clean up handles from previous effect run
    const prev = prefetchHandles.current;
    prefetchHandles.current = [];

    const uniqueUrls = [
      ...new Set(
        scenes
          .filter(s => s.clipId)
          .map(s => `/api/clips/${s.clipId}/video`)
      )
    ];

    const handles = uniqueUrls.map(url => {
      try {
        return prefetch(url, { method: 'blob-url', contentType: 'video/mp4' });
      } catch {
        return { free: () => {}, waitUntilDone: () => Promise.resolve() };
      }
    });

    prefetchHandles.current = handles;

    return () => {
      // Free prefetch handles only from previous run (not current)
      prev.forEach(h => h.free());
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scenes.map(s => s.clipId).join(',')]);
  ...
  const [clipTransitionEasing, setClipTransitionEasing] = useState<string>('out-expo');
  const [clipTransitionSfx, setClipTransitionSfx] = useState<string>('none');
  const [seqPattern, setSeqPattern] = useState<string[]>(['slide-left', 'zoom-in', 'fade']);
  ...
  const updateSceneTransitionEasing = (idx: number, easing: string) => {
    setScenes(prev => {
      const updated = [...prev];
      updated[idx] = { ...updated[idx], transitionEasing: easing };
      return updated;
    });
  };

  // Apply the sidebar transition + easing + sfx to ALL clips (functional update = no stale closure)
  const handleApplyTransitionsToAll = () => {
    setScenes(prev => prev.map(scene => ({
      ...scene,
      transition: clipTransition,
      transitionDuration: transitionDuration,
      transitionEasing: clipTransitionEasing,
      sfx: clipTransitionSfx
    })));
    setSuccess('✅ Transition + Easing + SFX applied to all scenes!');
    setTimeout(() => setSuccess(''), 2500);
  };

  // Apply only the sidebar transition + easing (no SFX change)
  const handleApplyTransitionOnlyToAll = () => {
    setScenes(prev => prev.map(scene => ({
      ...scene,
      transition: clipTransition,
      transitionDuration: transitionDuration,
    ...
    setSuccess('✅ Transition & Easing applied to all scenes!');
    setTimeout(() => setSuccess(''), 2500);
  };

  // Apply only the sidebar SFX (no transition change)
  const handleApplySfxOnlyToAll = () => {
    setScenes(prev => prev.map(scene => ({
      ...scene,
      sfx: clipTransitionSfx
    })));
    setSuccess('✅ SFX applied to all scenes!');
    setTimeout(() => setSuccess(''), 2500);
  };

  const addSeqPatternStep = (trans: string) => {
    if (!trans || trans === 'none') return;
    setSeqPattern(prev => [...prev, trans]);
  };

  const removeSeqPatternStep = (idx: number) => {
    setSeqPattern(prev => prev.filter((_, i) => i !== idx));
  };

  const handleApplySequentialPattern = () => {
    if (seqPattern.length === 0) return;
    setScenes(prev => prev.map((scene, idx) => {
      if (idx === prev.length - 1) return { ...scene, transition: 'none' };
      const stepTrans = seqPattern[idx % seqPattern.length];
      return {
        ...scene,
    ...
    setSuccess(`✅ Sequential pattern (${seqPattern.join(' ➔ ')}) applied across all clips with ${clipTransitionEasing} easing!`);
    setTimeout(() => setSuccess(''), 3000);
  };

  const applyPresetSequence = (pattern: string[]) => {
    setSeqPattern(pattern);
    setScenes(prev => prev.map((scene, idx) => {
      if (idx === prev.length - 1) return { ...scene, transition: 'none' };
      return {
        ...scene,
        transition: pattern[idx % pattern.length],
        transitionDuration: transitionDuration,
        transitionEasing: clipTransitionEasing
      };
    }));
    setSuccess(`✅ Sequential pattern (${pattern.join(' ➔ ')}) applied across all clips with ${clipTransitionEasing} easing!`);
    setTimeout(() => setSuccess(''), 3000);
  };

  // Inline helpers — called from per-clip → All buttons
  const applyTransitionToAll = (transition: string, duration: number, easing?: string) => {
    setScenes(prev => prev.map(scene => ({
      ...scene,
      transition,
      transitionDuration: duration,
      transitionEasing: easing || clipTransitionEasing
    })));
    setSuccess('✅ Transition & Easing applied to all scenes!');
    setTimeout(() => setSuccess(''), 2500);
  };

  const applySfxToAll = (sfx: string) => {
    setScenes(prev => prev.map(scene => ({
      ...scene,
      sfx
    })));
                      ...
                      <div style={{ borderTop: '1px solid var(--border-light)', paddingTop: '10px', marginTop: '4px' }}>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            style={{ margin: 0, fontSize: '11px', height: '28px', padding: '0 8px', flex: 1 }}
                          >
                            ...
                            <optgroup label="Blur Slide">
                              <option value="blur-slide-left">Blur Slide Left</option>
                              <option value="blur-slide-right">Blur Slide Right</option>
                              <option value="blur-slide-up">Blur Slide Up</option>
                              <option value="blur-slide-down">Blur Slide Down</option>
                            </optgroup>
                            <optgroup label="Pan">
                            ...
                            <optgroup label="Zoom">
                              ...
                              <option value="blur-zoom-in">Blur Zoom In</option>
                              <option value="blur-zoom-out">Blur Zoom Out</option>
                            </optgroup>
                            <option value="random">Random Mix</option>
                          </select>
                          <button
                            type="button"
                            title="Apply this transition to ALL clips"
                            onClick={() => applyTransitionToAll(scene.transition || 'none', scene.transitionDuration ?? transitionDuration, scene.transitionEasing ?? clipTransitionEasing)}
                            style={{
                              flexShrink: 0, padding: '0 5px', height: '28px', fontSize: '9px', fontWeight: 700,
                              background: 'var(--border-medium)', border: '1px solid var(--border-light)',
                              borderRadius: '4px', color: 'var(--text-gray)', cursor: 'pointer', whiteSpace: 'nowrap'
                            }}
                          >→ All</button>
                          </div>
                          {scene.transition && scene.transition !== 'none' && (
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginTop: '6px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <label style={{ fontSize: '9px', color: 'var(--text-gray)' }}>Dur:</label>
                                <input
                                  type="number"
                                  min="0.1"
                                  max="2.0"
                                  ...
                                  style={{ margin: 0, fontSize: '10px', height: '22px', padding: '0 4px', width: '40px', textAlign: 'center' }}
                                />
                                <span style={{ fontSize: '9px', color: 'var(--text-muted)' }}>s</span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <label style={{ fontSize: '9px', color: 'var(--text-gray)' }}>Ease:</label>
                                <select
                                  className="input-field"
                                  value={scene.transitionEasing || clipTransitionEasing}
                                  onChange={(e) => updateSceneTransitionEasing(idx, e.target.value)}
                                  style={{ margin: 0, fontSize: '9px', height: '22px', padding: '0 2px', flex: 1, minWidth: 0 }}
                                >
                                  <option value="out-expo">Exponential Out (DOTween)</option>
                                  <option value="in-out-expo">Exponential InOut</option>
                                  <option value="in-expo">Exponential In</option>
                                  <option value="out-back">Back / Overshoot Out</option>
                                  <option value="in-out-back">Back InOut</option>
                                  <option value="out-bounce">Bounce Out</option>
                                  <option value="out-elastic">Elastic / Spring Out</option>
                                  <option value="out-cubic">Cubic Out</option>
                                  <option value="out-quad">Quad Out</option>
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                            ...
                            <button
                              type="button"
                              title="Apply this SFX to ALL clips"
                              onClick={() => applySfxToAll(scene.sfx || 'none')}
                              style={{
                                flexShrink: 0, padding: '0 5px', height: '28px', fontSize: '9px', fontWeight: 700,
                                background: 'var(--border-medium)', border: '1px solid var(--border-light)',
                                borderRadius: '4px', color: 'var(--text-gray)', cursor: 'pointer', whiteSpace: 'nowrap'
          ...
          {sidebarTab === 'video' && (
            ...
            {clipTransition !== 'none' && (
              <div style={{ marginTop: '10px' }}>
                <label className="label">Transition Easing Curve (DOTween)</label>
                <select
                  className="input-field"
                  value={clipTransitionEasing}
                  onChange={(e) => setClipTransitionEasing(e.target.value)}
                  style={{ height: '34px', fontSize: '12px' }}
                >
                  <option value="out-expo">Exponential Out (DOTween default)</option>
                  <option value="in-out-expo">Exponential In-Out (Smooth S-Curve)</option>
                  <option value="in-expo">Exponential In (Rapid Acceleration)</option>
                  <option value="out-back">Back / Overshoot Out (Snappy Pop)</option>
                  <option value="in-out-back">Back In-Out (Anticipation + Overshoot)</option>
                  <option value="out-bounce">Bounce Out (Rubber Ball Bounce)</option>
                  <option value="out-elastic">Elastic / Spring Out (Rubber Band Spring)</option>
                  <option value="out-cubic">Cubic Out (Smooth Medium)</option>
                  <option value="out-quad">Quad Out (Soft Ease)</option>
            ...
            <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <button
                type="button"
                className="btn-secondary"
                onClick={handleApplyTransitionOnlyToAll}
              ...
              <button
                type="button"
                className="btn-secondary"
                onClick={handleApplySfxOnlyToAll}
                style={{ fontSize: '11px', width: '100%', justifyContent: 'center', padding: '6px' }}
              >
                Apply SFX to All Clips
              </button>
              <button
                type="button"
                className="btn-secondary"
                onClick={handleApplyTransitionsToAll}
                style={{ fontSize: '11px', width: '100%', justifyContent: 'center', padding: '6px', opacity: 0.7 }}
              >
                Apply Both to All Clips
              </button>
            </div>

            {/* Sequential Transition Pattern (A ➔ B ➔ C) */}
            <div style={{ marginTop: '16px', borderTop: '1px solid var(--border-light)', paddingTop: '14px' }}>
              <label className="label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>🔄 Sequential Pattern (A ➔ B ➔ C)</span>
                <span style={{ fontSize: '10px', color: 'var(--text-gray)' }}>{seqPattern.length} steps</span>
              </label>
              <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: '2px 0 8px' }}>
                Loops through selected transitions sequentially across clips (e.g. Slide Left ➔ Zoom In ➔ Fade).
              </p>

              {/* Active pattern chips */}
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '8px' }}>
                {seqPattern.map((trans, i) => (
                  <span key={i} style={{
                    display: 'inline-flex', alignItems: 'center', gap: '4px',
                    padding: '3px 8px', borderRadius: '12px', background: 'var(--border-medium)',
                    color: 'var(--text-white)', fontSize: '10px', fontWeight: 600
                  }}>
                    {i + 1}. {trans}
                    {seqPattern.length > 1 && (
                      <button
                        type="button"
                        onClick={() => removeSeqPatternStep(i)}
                        style={{ background: 'none', border: 'none', color: 'var(--text-gray)', cursor: 'pointer', padding: 0, fontSize: '11px', lineHeight: 1 }}
                      >×</button>
                    )}
                  </span>
                ))}
              ...
              <div style={{ display: 'flex', gap: '6px', marginBottom: '8px' }}>
                <select
                  ...
                  onChange={(e) => {
                    if (e.target.value) {
                      addSeqPatternStep(e.target.value);
                      e.target.value = '';
                    }
                  }}
                  style={{ height: '28px', fontSize: '11px', flex: 1, padding: '0 6px' }}
                >
                  <option value="" disabled>+ Add step to sequence...</option>
                  <option value="fade">Fade</option>
                  <option value="slide-left">Slide Left</option>
                  <option value="slide-right">Slide Right</option>
                  <option value="slide-up">Slide Up</option>
              ...
              <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '10px' }}>
                <button
                  type="button"
                  onClick={() => applyPresetSequence(['slide-left', 'slide-right'])}
                  style={{ fontSize: '9px', padding: '2px 6px', borderRadius: '4px', background: 'var(--border-light)', border: '1px solid var(--border)', color: 'var(--text-gray)', cursor: 'pointer' }}
                >⚡ Slide ⇄ Alternating</button>
                <button
                  type="button"
                  onClick={() => applyPresetSequence(['zoom-in', 'blur-slide-left', 'fade'])}
                  style={{ fontSize: '9px', padding: '2px 6px', borderRadius: '4px', background: 'var(--border-light)', border: '1px solid var(--border)', color: 'var(--text-gray)', cursor: 'pointer' }}
                >🎬 Cinematic Trio</button>
                <button
                  type="button"
                  onClick={() => applyPresetSequence(['pan-left', 'pan-right', 'slide-up', 'slide-down'])}
                  style={{ fontSize: '9px', padding: '2px 6px', borderRadius: '4px', background: 'var(--border-light)', border: '1px solid var(--border)', color: 'var(--text-gray)', cursor: 'pointer' }}
                >🚀 4-Way Motion</button>
              </div>
              ...
              <button
                type="button"
                className="btn-secondary"
                onClick={handleApplySequentialPattern}
                style={{ fontSize: '11px', width: '100%', justifyContent: 'center', padding: '7px', background: 'var(--primary)', color: 'var(--primary-foreground)', fontWeight: 600 }}
              >
                Apply Sequential Pattern (A ➔ B ➔ C) to All
              </button>
            </div>
